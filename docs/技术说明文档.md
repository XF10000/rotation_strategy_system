...
- 基于DCF估值的价值比判断
- **价值比计算**：当前价格 / DCF估值
- **卖出条件**：价值比 > 80% (`value_ratio_sell_threshold=0.80`)
- **买入条件**：价值比 < 70% (`value_ratio_buy_threshold=0.70`)

**维度2：RSI超买超卖（动态阈值）**
- **RSI周期**：14周 (`rsi_period=14`)
- **阈值类型**：使用行业特定的动态RSI阈值（124个申万二级行业）
- **极端阈值**：触及时无需背离确认，强制产生信号
- **普通阈值**：需要价格背离确认
- **背离检测**：比较最近3周的价格高点/低点与RSI高点/低点

**维度3：MACD动能确认**
- **MACD参数**：快线12周，慢线26周，信号线9周
  - `macd_fast=12`
  - `macd_slow=26`
  - `macd_signal=9`
- **卖出信号**：
  - 红色柱体（HIST>0）连续2根缩短 或
  - 柱体翻绿（HIST由正转负）或
  - DIF死叉DEA（DIF下穿DEA）
- **买入信号**：
  - 绿色柱体（HIST<0）连续2根缩短 或
  - 柱体翻红（HIST由负转正）或
  - DIF金叉DEA（DIF上穿DEA）

**维度4：极端价格+量能**
- **布林带参数**：20周期，2倍标准差
- **成交量均线**：4周均量（`volume_ma_period=4`）
- **卖出条件**：收盘价 ≥ 布林上轨 且 本周量 ≥ 4周均量 × 1.3
- **买入条件**：收盘价 ≤ 布林下轨 且 本周量 ≥ 4周均量 × 0.8

```python
# 极端价格+量能技术实现
def check_extreme_price_volume(self, data, indicators):
    """
    极端价格+量能信号判断
    ...
    volume_ma = indicators['volume_ma'].iloc[-1]  # 4周均量
    
    # 卖出信号
    if current_price >= bb_upper and current_volume >= volume_ma * 1.3:
        return 'SELL'
    
    # 买入信号
    if current_price <= bb_lower and current_volume >= volume_ma * 0.8:
        return 'BUY'
