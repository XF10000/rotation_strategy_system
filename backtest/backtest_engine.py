...
>>>>>>> dba34500e3e1ada8244293166cf2603a83f664f8
            ...
            for idx, row in filtered_weekly_data.iterrows():
                try:
                    ...
                    if 'macd' in row and 'macd_signal' in row and 'macd_histogram' in row:
                        ...
                        macd_signal_val = float(row['macd_signal']) if pd.notna(row['macd_signal']) else 0.0
                        macd_histogram_val = float(row['macd_histogram']) if pd.notna(row['macd_histogram']) else 0.0
                        ...
                        # 如果没有MACD数据，添加默认值
                        macd_points.append([timestamp, 0.0])
                        macd_signal_points.append([timestamp, 0.0])
                        macd_histogram_points.append([timestamp, 0.0])
                except Exception as e:
